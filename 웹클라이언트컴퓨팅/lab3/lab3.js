...
const outputA = _.sortBy(memberObj, (item) => item.name);
console.log(outputA);

//(문제2-1) 객체 이름(name)으로 정렬. 아래코드를 올바로 수정할 것.
console.log('\n- 객체 주민번호(rrid)로 정렬');
const outputB = _.sortBy(memberObj, (item) => item.rrid);
console.log(outputB);
...
memberObj3.push(_.object([['name', '성공함'], ['weight', 67], ['rrid', '950810-3306548']]));
...
mappedMemberObj = memberObj3.map(function (item) {_.extend(item, {'성별':(item.rrid.substr(7,1) == 1 || item.rrid.substr(7,1) == 3)? '남자' : '여자'})
...
cnt2 = memberObj3.map(
                ...
                function (item) {
                  return item.성별 == "여자"; /* true를 수정할 것 */ }
             ).reduce(
                function (previous, current) {
                  return previous + 1; // 0을 수정할 것
                }, 0);
console.log('\n여성 = ' + cnt2 + '명');
...
Member.prototype.성별 = function() {
  // 이 위치에 구현
  this.성별 = ((parseInt(this.rrid.charAt(7))%2)==1)? "남자" : "여자"
}
...
memberObj4.forEach((item, index) => { item.성별();     });
console.log('\n(memberObj4)');
console.log(memberObj4);
