"""
 Date: 2017/11/26
 file name: go_any.py
 Purpose: 이 코드는 구동체를 전,후진,회전을 시키기 위한 코드이다.
"""
...
def go(direction, Rspeed, Lspeed, running_time):
	"""
	go함수는 바퀴의 방향인 direction(forward, backward)를 입력받고 양쪽 바퀴의 속도와 작동시간을 입력받고
	작동시간만큼 입력받은 방향으로 구동체를 움직이는 함수이다.
	"""
...
def PointTurn(direction, speed, running_time):
	"""
	PointTurn함수는 양쪽 바퀴의 방향을 다르게하여 구동체를 회전시키는 함수로
	회전하는 방향과 속도, 모터의 작동시간을 입력받는다.
	"""
    if direction == 'left':
...
def linetrace(direction, track, speed):
	"""
	linetrace함수는 바퀴의 회전방향과 5방향 추적센서에서 파악한 라인상의 구동체의 위치와 속도를 입력받아 그에 알맞게
	입력받은 속도를 조절하며 구동체가 라인을 따라가게 만드는 함수이다.
	""" 
